...
For interactive CLI program verification, Oracle CAN use PTY tools:
- **pty_spawn**: Run commands/tests in interactive terminal (e.g., npm test, bun run, cargo test)
- **pty_read**: Read command output to verify results
- **pty_write**: Send input to interactive programs if needed
- **pty_snapshot**: Capture terminal screen state

**IMPORTANT**: When using pty_spawn:
- Always expand the home directory (e.g., DOLLAR_SIGN_HOME/project or any path starting with tilde)
- Use absolute paths for all file/directory references
- Verify actual program behavior matches expected behavior
